 ...
 * This mapping is represented as a sequence of linear models (fragments) which, if @p EpsilonRecursive is not zero, are
 ...
 */
    ...
    protected:
        ...
        using PlaType = neats::PiecewiseOptimalModel<K, uint32_t, double, Floating, double, true>;

        size_t n;                           ///< The number of elements this index was built on.
        K first_key;                        ///< The smallest element.
        std::vector<typename PlaType::LinearBuilderType::template Linear<float>> fragments;      ///< The fragments composing the index.
        std::vector<size_t> levels_offsets; ///< The starting position of each level in fragments[], in reverse order.
        ...
        template<typename RandomIt, typename FragmentsType = PlaType::LinearBuilderType::template Linear<float>>
        static void build(RandomIt first, RandomIt last,
                          size_t epsilon, size_t epsilon_recursive,
                          std::vector<FragmentsType> &fragments,
            ...
            fragments.reserve(n / (epsilon * epsilon));
            ...
            auto build_level = [&](auto epsilon, auto in_fun, auto out_fun, size_t last_n) {
                ...
                if (fragments.back() == sentinel)
                    --n_segments;
                else {
                    if (fragments.back()(sentinel - 1) < last_n)
                        fragments.emplace_back(*std::prev(last) + 1, 0, last_n); // Ensure keys > last are mapped to last_n
                    fragments.emplace_back(sentinel, 0, last_n);
                }
                return n_segments;
            };
            */
            auto build_level = [&](auto epsilon, auto in_fun, auto begin, auto end, size_t last_n) {
                auto linear_fragments = neats::MakeLinearFragmentation<PlaType>(epsilon, in_fun, begin, end);
                auto n_fragments = linear_fragments.size();
                for (auto &&fragment: linear_fragments) {
                    fragments.emplace_back(fragment.template result<typename PlaType::LinearBuilderType::template Linear<float>>());
                    //fragments.emplace_back(segment);
                }
            ...
            auto last_n = build_level(epsilon, identity, first, last, n);
            levels_offsets.push_back(fragments.size());
            ...
            while (epsilon_recursive && last_n > 1) {
                ...
                //auto in_fun_rec = [&](auto i) { return fragments[offset + i].key; };
                auto transform_value = [](auto&& fragment_it) -> auto { return (fragment_it)->key; };
                last_n = build_level(epsilon_recursive, transform_value, fragments.begin() + offset, fragments.end(),
                                     last_n);
                levels_offsets.push_back(fragments.size());
            }
        }
        ...
        auto segment_for_key(const K &key) const {
            if constexpr (EpsilonRecursive == 0) {
                return std::prev(std::upper_bound(fragments.begin(), fragments.begin() + segments_count(), key));
            }

            auto it = fragments.begin() + *(levels_offsets.end() - 2);
            for (auto l = int(height()) - 2; l >= 0; --l) {
                auto level_begin = fragments.begin() + levels_offsets[l];
                //auto pos = std::min<size_t>((*it)(key), std::next(it)->intercept);
                auto _k = (*it)(key);
                // gen = i + 1 =>
                auto pos = std::min<size_t>(_k < 0? size_t(_k * -1) : size_t(_k), std::next(it)->template get_intercept<size_t>());
                auto lo = level_begin + PGM_SUB_EPS(pos, EpsilonRecursive + 1);

                static constexpr size_t linear_search_threshold =
                        8 * 64 / sizeof(typename PlaType::LinearBuilderType::template Linear<float>);
    ...
    public:
        ...
        PGMIndex(RandomIt first, RandomIt last)
                : n(std::distance(first, last)),
                  ...
                  fragments(),
                  levels_offsets() {
            build(first, last, Epsilon, EpsilonRecursive, fragments, levels_offsets);
        }
        ...
        /**
         * Returns the number of fragments in the last level of the index.
         * @return the number of fragments
         */
        size_t segments_count() const { return fragments.empty() ? 0 : levels_offsets[1] - 1; }
        ...
        size_t num_segments() const { return fragments.size(); }
        ...
        size_t size_in_bytes() const {
            return fragments.size() * sizeof(PlaType::LinearModelType) + levels_offsets.size() * sizeof(size_t);
        }
    };
