...
int main(int const argc, char const ** argv)
{
    ...
    for (size_t i = 0; i < testcasecount; i++)
    {
        ...
        {
            cerr << "ERROR: vector from file " << file << " with type " << type << " could not be loaded" << endl;
            displayUsage(argv[0]);
            return 1;
        }
...
bool runSingleTest(int_vector<> const & testcase, iv_testresult & result)
{
    ...
    auto stop = timer::now();
    result.enc_MBperSec = size_in_mega_bytes(testcase) / duration_cast<seconds>(stop - start).count();

    // care for compression rate
    result.comp_percent = size_in_mega_bytes(test) / size_in_mega_bytes(testcase) * 100.0;
...
bool runTestcase(int_vector<> const & testcase, iv_testresult * result)
{
    ...
    for (i = 0; i < vectorcount; i++)
    {
        if (!testfine[i])
        {
            cerr << "Test on Vector " << vectornames[i] << "failed" << endl;
            testsfine = false;
        }
    }
    return testsfine;
...
void displayUsage(char const * pname)
{
    ...
    cerr << "\tProgram will test a couple of compression vectors " << endl
         ...
         << "\tfor each testcase." << endl
         << "\tAdditionally, an overall result on different " << endl
         << "\tcompression vectors is printed." << endl;
    cerr << "\tThe generated output uses a CSV format, so "
         << "you may save it to a csv file for better visability" << endl
         << "\tand other utilites." << endl;
...
void displayHeading()
{
    ...
    cout << setw(20) << "testcase" << setw(1) << ";" << setw(20) << "vector" << setw(1) << ";" << setw(20)
         << "encodingrate" << setw(1) << ";" << setw(20) << "decodingrate" << setw(1) << ";"
...
void displayResult(char const * testcase, iv_testresult const * result)
{
    ...
    for (size_t i = 0; i < vectorcount; i++)
    {
        cout << setw(20) << testcase << setw(1) << ";" << setw(20) << vectornames[i] << setw(1) << ";" << setw(20)
             << result[i].enc_MBperSec << setw(1) << ";" << setw(20) << result[i].dec_MBperSec << setw(1) << ";"
             << result[i].comp_percent << endl;
    }
}
